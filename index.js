  ...
  try {
    ...
    if (result) {
      ...
      let info = await transporter.sendMail({
        ...
        html: `<a href="https://url-shrtn.herokuap.com/auth/${req.body.email}/${string}">Click on this link </a>`, // html body
      });
      await db
        .collection("users")
...
app.get("/auth/:mail/:string", async (req, res) => {
  try {
    ...
    if (result.string == req.params.string) {
      res.redirect(
        `https://url-shrtnr.netlify.app/rese.html?${req.params.mail}?${req.params.string}`
      );
